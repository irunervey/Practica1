...
def mi_busqueda(problem, fringe):
    """Search through the successors of a problem to find a goal.
    The argument fringe should be an empty queue.
    If two paths reach a state, only use the best one. [Fig. 3.18]"""
    closed = {}
...
def graph_ramificacion_search(problem):
...
def ramificacionYSalto(problem):
    ...
    """
    return mi_busqueda(problem, miQueue())

    pass
def ramificacionYSaltoConSubestimacion(problem):

    return mi_busqueda(problem, miQueue0(problem))
